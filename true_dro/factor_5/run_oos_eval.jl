"""
run_oos_eval.jl — Additive factor model에서 nom vs rob OOS Phase B 평가.

x_nom, x_rob를 받아서 asymmetric Dirichlet OOS로 비교.
gap = rob - nom (leader minimizes max-flow이므로 gap < 0 → rob 승).

Usage:
    include("true_dro/factor_3/run_oos_eval.jl")
"""

using JuMP, HiGHS, Gurobi, Printf, Dates, Statistics, Random, Distributions, LinearAlgebra

include("../../network_generator.jl")
using .NetworkGenerator

include("../../oos_evaluation.jl")        # build_maxflow_template, solve_deterministic_maxflow!
include("../oos_evaluate.jl")             # solve_follower_weighted, compute_maxflow_per_scenario

# ============================================================
# Helper: 간단한 2-model Phase B evaluation
# ============================================================
function oos_phase_b_two_models(x_nom, x_rob, network, capacities, v, w;
                                  β=0.5, M=200, noise_scale=0.5, seed=42)
    K = size(capacities, 2)
    rng = MersenneTwister(seed)

    costs_nom = Vector{Float64}(undef, M)
    costs_rob = Vector{Float64}(undef, M)

    for m in 1:M
        # Asymmetric α → p_center ≠ uniform
        α = β .* (1.0 .+ noise_scale * randn(rng, K))
        α = max.(α, 0.01)
        p_center = α / sum(α)

        # Nominal
        h_nom = solve_follower_weighted(network, x_nom, v, w, capacities, p_center)
        flows_nom = compute_maxflow_per_scenario(network, x_nom, h_nom, v, capacities)
        costs_nom[m] = dot(p_center, flows_nom)

        # Robust
        h_rob = solve_follower_weighted(network, x_rob, v, w, capacities, p_center)
        flows_rob = compute_maxflow_per_scenario(network, x_rob, h_rob, v, capacities)
        costs_rob[m] = dot(p_center, flows_rob)

        if m % 50 == 0
            @printf("  OOS outer %d/%d\n", m, M)
        end
    end

    gap = costs_rob .- costs_nom   # < 0 → rob wins (leader minimizes max-flow)

    return Dict(
        :gap_mean   => mean(gap),
        :gap_p5     => quantile(gap, 0.05),
        :gap_p95    => quantile(gap, 0.95),
        :rob_wins   => mean(gap .< 0),    # leader minimizes: rob < nom → rob wins
        :nom_mean   => mean(costs_nom),
        :rob_mean   => mean(costs_rob),
        :costs_nom  => costs_nom,
        :costs_rob  => costs_rob,
    )
end

# ============================================================
# Network configs: (generator, all_intd?, γ, name)
# ============================================================
configs = [
    (:polska,   generate_polska_network,   true,  1, "polska_allintd_γ1"),
    (:abilene,  generate_abilene_network,  true,  1, "abilene_allintd_γ1"),
    (:nobel_us, generate_nobel_us_network, true,  2, "nobel_us_allintd_γ2"),
]

# x* from previous runs
known_x = Dict(
    :polska   => (nom_arcs=[18], rob_arcs=[33]),
    :abilene  => (nom_arcs=[11], rob_arcs=[5]),
    :nobel_us => (nom_arcs=[7, 25], rob_arcs=[15, 20]),
)

v = 1.0
S = 20
λU = 2.0

println("=" ^ 80)
println("OOS Phase B Evaluation — Additive Factor Model (nom vs rob)")
println("  β values: [0.1, 0.3, 0.5, 1.0]")
println("  M=200, noise_scale=0.5, seed=42")
println("=" ^ 80)

all_results = Dict()

for (net_key, gen_func, all_intd, γ, label) in configs
    println("\n" * "#" ^ 70)
    @printf("# %s (γ=%d, all_intd=%s)\n", net_key, γ, all_intd)
    println("#" ^ 70)
    flush(stdout)

    net = gen_func()
    num_arcs = length(net.arcs) - 1

    if all_intd
        intd_flags = fill(true, length(net.arcs))
        net = RealWorldNetworkData(net.name, net.original_node_names, net.nodes, net.arcs,
            net.N, intd_flags, net.arc_adjacency, net.node_arc_incidence)
    end

    caps, _ = generate_capacity_scenarios_factor_sparse(length(net.arcs), S;
        interdictable_arcs=net.interdictable_arcs, seed=42, num_factors=5)
    intd_idx = findall(net.interdictable_arcs[1:num_arcs])
    w = round(maximum(caps[intd_idx, :]); digits=4)

    # Build x vectors from known arcs
    kx = known_x[net_key]
    x_nom = zeros(Float64, num_arcs)
    for a in kx.nom_arcs; x_nom[a] = 1.0; end
    x_rob = zeros(Float64, num_arcs)
    for a in kx.rob_arcs; x_rob[a] = 1.0; end

    @printf("  x_nom=%s, x_rob=%s, w=%.4f\n", string(kx.nom_arcs), string(kx.rob_arcs), w)

    for β in [0.1, 0.3, 0.5, 1.0]
        @printf("\n  β=%.1f:\n", β)
        flush(stdout)

        result = oos_phase_b_two_models(x_nom, x_rob, net, caps, v, w;
                                          β=β, M=200, noise_scale=0.5, seed=42)

        all_results[(net_key, β)] = result

        @printf("    nom_mean=%.4f, rob_mean=%.4f\n", result[:nom_mean], result[:rob_mean])
        @printf("    gap(rob-nom): mean=%.4f, [p5,p95]=[%.4f,%.4f], rob_wins=%.1f%%\n",
                result[:gap_mean], result[:gap_p5], result[:gap_p95], result[:rob_wins]*100)
        flush(stdout)
    end
end

# ---- Summary table ----
println("\n" * "=" ^ 90)
println("OOS Phase B Summary (gap = rob - nom, <0 → robust wins)")
println("=" ^ 90)
@printf("%-20s %4s  %10s  %10s  %10s  %10s  %8s\n",
        "Network", "β", "Nom Mean", "Rob Mean", "Gap Mean", "Gap p95", "Rob Win%")
println("-" ^ 90)

for (net_key, _, _, γ, label) in configs
    for β in [0.1, 0.3, 0.5, 1.0]
        r = all_results[(net_key, β)]
        @printf("%-20s %4.1f  %10.4f  %10.4f  %10.4f  %10.4f  %7.1f%%\n",
                label, β, r[:nom_mean], r[:rob_mean], r[:gap_mean], r[:gap_p95], r[:rob_wins]*100)
    end
end
println("=" ^ 90)
println("\nDone! $(now())")
