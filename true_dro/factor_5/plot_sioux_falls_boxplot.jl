"""
plot_sioux_falls_boxplot.jl — Sioux Falls all-intd γ=2 λU=2.0, Nominal vs Robust.
x_nom=[72,73] vs x_rob=[35,73]
oos_phase_b_generic (same_alpha, diff_alpha) + boxplot.
"""
...
# ============================================================
# Sioux Falls setup — all-intd, γ=2, λU=2.0
# ============================================================
...
@printf("Sioux Falls: arcs=%d, γ=2, w=%.4f, x_nom=[72,73], x_rob=[35,73]\n", num_arcs, w)

β_list = [0.5, 1.0, 5.0]

for mode in [:symmetric, :same_alpha, :diff_alpha]
    println("\n" * "=" ^ 80)
    @printf(">>> Sioux Falls — mode=%s\n", mode)
    println("=" ^ 80)

    for β in β_list
        @printf("\n--- SiouxFalls β=%.1f mode=%s ---\n", β, mode)
        flush(stdout)

        costs = oos_phase_b_generic(x_dict, net_mod, caps, 1.0, w;
                                     β=β, M=500, seed=42, mode=mode)

        fname = @sprintf("true_dro/factor_5/plots/sioux_falls_boxplot_%s_beta%.1f.png", mode, β)
        title = @sprintf("Sioux Falls OOS [%s] (β=%.1f, γ=2)", mode, β)
