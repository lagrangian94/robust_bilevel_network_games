"""
test_polska_k3.jl — polska k=3, γ=3: nominal SP + robust DRO 비교.
Nominal은 build_full_2SP_model (LP), Robust은 Benders (ε=1.0).
"""
...
caps, F_mat = generate_capacity_scenarios_factor_model(length(net.arcs), 20;
    interdictable_arcs=net.interdictable_arcs, seed=42, num_factors=7)
...
@printf("polska k=3: arcs=%d, intd=%d, γ=%d, w=%.4f\n", num_arcs, length(intd_idx), γ, w)
flush(stdout)

# ============================================================
# Nominal SP (build_full_2SP_model, LP)
# ============================================================
println("\n" * "="^60)
println("Nominal SP (LP)")
println("="^60)
flush(stdout)
...
# ============================================================
# Robust DRO (ε=1.0, Benders)
# ============================================================
println("\n" * "="^60)
println("Robust DRO (ε=1.0, Benders)")
println("="^60)
flush(stdout)
...
serialize(joinpath(@__DIR__, "results", "polska_k3_gamma3_test.jls"), Dict(
    :x_nom => x_nom, :Z0_nom => Z0_nom, :wt_nom => wt_nom,
    :x_rob => Float64.(x_rob), :Z0_rob => res_rob[:Z0], :wt_rob => wt_rob,
    :same => x_nom_int == x_rob,
))
println("Saved to results/polska_k3_gamma3_test.jls")
