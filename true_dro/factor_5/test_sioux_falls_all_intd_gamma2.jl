"""
test_sioux_falls_all_intd.jl — sioux_falls, all arcs interdictable, additive factor k=5, γ=1
"""
...
@printf("sioux_falls ALL intd k=5: arcs=%d, intd=%d, γ=%d, w=%.4f\n", num_arcs, length(intd_idx), γ, w)
flush(stdout)

# Nominal (Benders ε=0)
println("\n" * "="^60)
println("Nominal (Benders ε=0)")
println("="^60)
flush(stdout)
