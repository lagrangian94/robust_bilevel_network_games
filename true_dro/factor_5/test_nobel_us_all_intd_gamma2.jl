"""
test_nobel_us_all_intd.jl — nobel_us, all arcs interdictable, additive factor k=5, γ=1
"""
...
caps, F_mat = generate_capacity_scenarios_factor_sparse(length(net_mod.arcs), 20;
...
@printf("nobel_us ALL intd k=5: arcs=%d, intd=%d, γ=%d, w=%.4f\n", num_arcs, length(intd_idx), γ, w)
flush(stdout)
...
model_nom, vars_nom = build_full_2SP_model(net_mod, S, λU, λU, γ, w, 1.0, uncertainty_set)
