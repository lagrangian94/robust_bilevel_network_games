...
model_s, vars_s = build_omp(network, ϕU, λU, γ, w; optimizer=Gurobi.Optimizer)
result_s = strict_benders_optimize!(model_s, vars_s, network, ϕU, λU, γ, w, uncertainty_set;
    optimizer=Gurobi.Optimizer, max_iter=5, πU=πU, yU=yU, ytsU=ytsU)
...
outer_cut_hybrid = evaluate_master_opt_cut(
    leader_instances, follower_instances, isp_data, hybrid_cut_info, 1)
