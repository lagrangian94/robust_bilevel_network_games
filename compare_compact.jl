...
wm, wv = build_omp(network, ϕU, λU, γ, w; optimizer=Gurobi.Optimizer, multi_cut=multi_cut)
tr_nested_benders_optimize!(wm, wv, network, ϕU, λU, γ, w, warm_uset;
    ...
    multi_cut=multi_cut, outer_tr=false, inner_tr=true)
...
for test_S in [1, 2]
    ...
    GC.gc()
    model_o, vars_o = build_omp(network, ϕU, λU, γ, w; optimizer=Gurobi.Optimizer, multi_cut=multi_cut)
    t_start = time()
...
@eval Main function evaluate_master_opt_cut(isp_leader_instances::Dict, isp_follower_instances::Dict, isp_data::Dict, cut_info::Dict, iter::Int; multi_cut=false)
    return $evaluate_master_opt_cut_compact(isp_leader_instances, isp_follower_instances, isp_data, cut_info, iter; multi_cut=multi_cut)
end
...
for test_S in [1, 2]
    ...
    GC.gc()
    model_c, vars_c = build_omp(network, ϕU, λU, γ, w; optimizer=Gurobi.Optimizer, multi_cut=multi_cut)
    t_start = time()
