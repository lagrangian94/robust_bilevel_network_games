...
function build_dualized_outer_subproblem(network, S, ϕU, γ, w, v, uncertainty_set, λ, x, h, ψ0; optimizer=nothing)
    ...
    + [-1*(I_0*Zhat1_1[s,:,:] + Zhat1_3[s,:,:]) + Zhat2[s,:,:] I_0*βhat1_1[s,:] + βhat1_3[s,:] - βhat2[s,:]] .== 0)
    # --- From Φhat
    @constraint(model, [s=1:S], [v*Mhat[s,1:num_arcs, 1:num_arcs] v*Mhat[s,1:num_arcs, num_arcs+1]] - Uhat1[s,:,:] - Uhat2[s,:,:] + Uhat3[s,:,:] .== 0)
    # --- From Φtilde ---
    @constraint(model, [s=1:S], [-Mtilde[s,1:num_arcs, 1:num_arcs] -Mtilde[s, 1:num_arcs, num_arcs+1]] + Utilde2[s,:,:] - Utilde3[s,:,:]
    + [-1*(I_0*Ztilde1_1[s,:,:] - Ztilde1_5[s,:,:]) + Ztilde2[s,:,:] I_0*βtilde1_1[s,:] + βtilde1_5[s,:] - βtilde2[s,:]] .== 0)
    # --- From Ψtilde ---
    @constraint(model, [s=1:S], [v*Mtilde[s,1:num_arcs, 1:num_arcs] v*Mtilde[s,1:num_arcs, num_arcs+1]] - Utilde1[s,:,:] - Utilde2[s,:,:] + Utilde3[s,:,:] .== 0)
    # --- From Ytilde_ts ---
    @constraint(model, [s=1:S], Mtilde[s,1, 1:num_arcs+1]' - [N_ts'* Ztilde1_2[s,:,:] -N_ts'* βtilde1_2[s,:]] .== 0)
    # --- From μhat ---
    @constraint(model, [s=1:S, k=1:num_arcs], βhat2[s,k] == α[k])
    # --- From μtilde ---
    @constraint(model, [s=1:S, k=1:num_arcs], βtilde2[s,k] == α[k])
    # --- From Πhat ---
    @constraint(model, [s=1:S], [N*Zhat1_1[s,:,:] + Zhat1_2[s,:,:] zeros(num_nodes-1)] + [zeros(num_nodes-1, num_arcs) βhat1_2[s,:]] .==0.0)
    # --- From Πtilde ---
    @constraint(model, [s=1:S], [N*Ztilde1_1[s,:,:] + Ztilde1_4[s,:,:] zeros(num_nodes-1)] + [zeros(num_nodes-1, num_arcs) βtilde1_4[s,:]] .==0.0)
    # --- From Ytilde ---
    @constraint(model, [s=1:S], [N_y'*Ztilde1_2[s,:,:] + Ztilde1_3[s,:,:].-Ztilde1_6[s,:,:] -N_y'*βtilde1_2[s,:]-βtilde1_3[s,:].+βtilde1_6[s,:]] .==0.0)
    # --- From Λhat1 ---
